...
def main():
    ...
    date_options = {
        "Last 3 Years": timedelta(days=1095),  # Made this first
    ...
    selected_range = st.sidebar.radio("Select Time Period", 
                                    ...
                                    index=0)  # Default to "Last 3 Years"
    ...
    st.sidebar.subheader("🏢 Line of Business Selection")
    
    # Special WC filter
    show_wc_only = st.sidebar.checkbox("Show Workers Compensation Only", value=False)
    
    if show_wc_only:
        selected_lobs = ['WC']
    else:
        all_lobs = sorted(df['LOB'].unique())
        selected_lobs = st.sidebar.multiselect(
